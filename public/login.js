...
function send_key(pass) {
	const raw = new Uint16Array([].map.call(pass, function(c) {
		return c.charCodeAt(0)
	})).buffer;
	const keyConfig = {name: "RSA-OAEP", modulusLength: 4096, publicExponent: new Uint8Array([1,0,1]), hash: "SHA-256"};
	crypto.subtle.generateKey(keyConfig, true, ["encrypt", "decrypt"]).then((key) => {
